...
def calc_eval_result(output_path, gt_file, num_chunks, data_path):
    ...
    for sample in pred_contents:
        ...
        iou, precision, recall = cal_iou_precision_recall(sample['answer'], pred_list)
        if iou == -2:
        ...
        # parse zoom calls
        zoom_calls_list = extract_zoom_calls(sample['pred'])
        zoom_ious = []
        for idx, zoom_call in enumerate(zoom_calls_list):
            iou, precision, recall = cal_iou_precision_recall(sample['answer'], zoom_call)
            zoom_ious.append(iou)
        prediction_set[sample['id']].update({
            f'iou_zooms': zoom_ious,
        })
        max_zoom_round = max(max_zoom_round, len(zoom_ious))
        # parse meta infos
    ...
    for key, result in prediction_set.items():
        ...
        for idx, iou in enumerate(result['iou_zooms']):
            meter_dic[f"miou_zoom_{idx}"].add_score(iou, 'yes')
            meter_dic[f"miou_zoom_{idx}_part"].add_score(iou, 'yes')
        for idx in range(len(result['iou_zooms']), max_zoom_round):
            meter_dic[f"miou_zoom_{idx}"].add_score(0, 'yes')
