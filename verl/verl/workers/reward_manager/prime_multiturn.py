...
async def parallel_compute_score_async(evaluation_func, completions, references, tasks, extra_info=None, num_processes=64):
    ...
    for result, completion, reference, task in zip(results, completions, references, tasks):
        if isinstance(result, Exception) or result is None:
            ...
            raise ValueError(f"In prime_multiturn, Unknown type {type(result)} for result: {result}")
    return scores
...
@register("prime_multiturn")
class PrimeMultiturnRewardManager:
    """
    ...
    def verify(self, data):
        ...
        response_mask = data.batch["loss_mask"][:, prompt_length:]
        response_ids = data.batch["responses"].masked_fill(response_mask == 0, self.tokenizer.pad_token_id)
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        # batched scoring
        sequence_ids = data.batch["input_ids"]
        sequence_strs = self.tokenizer.batch_decode(sequence_ids, skip_special_tokens=True)
        prompt_ids = data.batch["prompts"]
        prompt_length = prompt_ids.shape[-1]
        response_mask = data.batch["loss_mask"][:, prompt_length:]
        response_ids = data.batch["responses"].masked_fill(response_mask == 0, self.tokenizer.pad_token_id)
        ...
        for i in range(len(data)):
            ...
            if already_print_data_sources[data_source] < self.num_examine:
                ...
                score = reward
                print("[prompt_completion]", sequence_strs[i])
                print("[response]", response_strs[i])
                print("[ground_truth]", ground_truth)
                print("[score]", score)
                print("[reward_extra_info]", scores[i].items())
