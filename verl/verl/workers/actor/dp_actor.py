...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if "multi_modal_inputs" in micro_batch.keys():
            for key in ["pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw", "second_per_grid_ts"]:  # not include "input_ids", "attention_mask" which is partial
                tensor_li = [inputs[key] for inputs in micro_batch["multi_modal_inputs"] if key in inputs]
                if len(tensor_li) > 0:
                    multi_modal_inputs[key] = torch.cat(tensor_li, dim=0)
