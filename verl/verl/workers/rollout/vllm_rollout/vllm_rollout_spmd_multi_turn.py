...
class vLLMRolloutMultiTurn(BaseRollout):
    ...
    async def execute_tools(self, tool_call_arguments, tools_kwargs):
    ...
    async def gather_tools(self, tool_exec_tasks):
        return await asyncio.gather(*tool_exec_tasks)
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with _timer("generation/preprocess", generation_timing):
            ...
            for messages, multi_modal_data, mm_processor_kwargs, tools_kwarg in zip(non_tensor_batch.pop("raw_prompt"), non_tensor_batch.pop("multi_modal_data"), non_tensor_batch.pop("mm_processor_kwargs"), non_tensor_batch.pop("tools_kwargs")):
                ...
                tools_kwarg.update({
                    "max_frames": 64,
                    "video": multi_modal_data["video"][0],
                })
                    ...
                    for i, idx in enumerate(active_id_list):
                        ...
                        if step < self.max_turns:
                            ...
                            if res["type"] == "tool":
                                ...
                                tool_exec_tasks.append(self.execute_tools(tool_call_arguments, tools_kwargs[idx]))
                                tool_exec_ids.append(i)
                                tool_result = ""
                            ...
                            tool_result_list[i] = tool_result
                if step < self.max_turns:
                    # async execute tool call
                    with _timer("generation/execute_tool", generation_timing):
                        tool_exec_results = asyncio.run(self.gather_tools(tool_exec_tasks))
                        for i, act_id in enumerate(tool_exec_ids):
                            tool_res = tool_exec_results[i]
                            idx = active_id_list[act_id]
                            if tool_res["type"] == "result":
                                tool_result = tool_res["content"]
                                valid_tool_exec_stats[idx] += 1
                            elif tool_res["type"] == "error":
                                tool_result = tool_res["content"]
                            tool_result_list[act_id] = tool_result
                    # postprocess tool call
                    with _timer("generation/postprocess_tool", generation_timing):
                        for i, idx in enumerate(active_id_list):
                            tool_result = tool_result_list[i]
                            if tool_result != "":
                                if isinstance(tool_result, str):
                                    ...
                                    new_fps = [ vllm_inputs[idx]["mm_processor_kwargs"]["fps"][-1]]
