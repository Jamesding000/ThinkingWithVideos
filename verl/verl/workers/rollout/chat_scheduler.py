...
class CompletionCallback(ABC):
    def __init__(self, config: DictConfig, scheduler: "ChatCompletionScheduler"):
        ...
        print(f"Initialized tool_schemas: {self.tool_schemas}", flush=True)

        local_path = copy_to_local(config.actor_rollout_ref.model.path)
        self.tokenizer = hf_tokenizer(local_path, trust_remote_code=True)
        print(f"Initialized tokenizer: {self.tokenizer}", flush=True)
        self.processor = hf_processor(local_path, trust_remote_code=True, use_fast=True)
        print(f"Initialized processor: {self.processor}", flush=True)
        print(f"Initialized processor chat_template: {self.processor.chat_template}", flush=True)
...
class ToolCompletionCallback(CompletionCallback):
    ...
    async def __call__(self, 
        ...
        if self.max_turns and len(messages) >= self.max_turns:
            logger.info(f"[id={completions.id}, turn={len(messages)}, finish_reason={finish_reason}] Reach max turns, done!")
            return

        # STEP 1: check if the model called tools
        if finish_reason != "tool_calls":
            logger.info(f"[id={completions.id}, turn={len(messages)}, finish_reason={finish_reason}] No tool called, done!")
            return

        # STEP 2: call tools
        tool_calls = completions.choices[0].message.tool_calls
        logger.info(f"[id={completions.id}, turn={len(messages)}, finish_reason={finish_reason}] Call {len(tool_calls)} tools")
        tasks = []
        for tool_call in tool_calls:
            tasks.append(self._call_tool(tool_call))
        tool_responses = await asyncio.gather(*tasks)
        if any(isinstance(item, Exception) for item in tool_responses):
            logger.info(f"[id={completions.id}, turn={len(messages)}, finish_reason={finish_reason}] Error when calling tools, done!")
            return
        messages.extend(tool_responses)

        # STEP 3: resubmit completion request with tool responses
        self.scheduler.submit_chat_completions(messages=messages, extra_body=extra_body, request_id=completions.id, info=info)
    ...
    def postprocess(self, batch: DataProto, batch_conversations: List[List[Dict[str, str]]], n: int) -> DataProto:
        ...
        def convert_message_to_qwen_format(messages):
            new_messages = copy.deepcopy(messages)
            for message in new_messages:
                for part_message in message['content']:
                    if not isinstance(part_message, dict) or 'type' not in part_message:
                        continue
                    if part_message['type'] == 'video_url':
                        part_message['type'] = 'video'
            return new_messages
        
        # NOTE: tolist() is a must, since processor requires type==list, but tokenizer not
        prompts = [self.processor.apply_chat_template(prompt, tools=self.tool_schemas, add_generation_prompt=True, tokenize=False) for prompt in batch.non_tensor_batch["raw_prompt"].tolist()]
        
        assert len(batch_conversations) == len(prompts) * n

        # sequences: [prompt + response]
        sequences = [self.processor.apply_chat_template(convert_message_to_qwen_format(conversation), tools=self.tool_schemas, add_generation_prompt=False, tokenize=False) for conversation in batch_conversations]

        # responses: [response]
        responses = [sequence[len(prompts[i // n]) :] for i, sequence in enumerate(sequences)]

        # prompts = self.processor(text=prompts, return_tensors="pt", padding="longest", padding_side="left")
        # data reuse
        multi_modal_inputs = {}
        micro_batch = batch.non_tensor_batch
        if "multi_modal_inputs" in micro_batch.keys():
            # print(f'micro_batch["multi_modal_inputs"]={micro_batch["multi_modal_inputs"]}')
            for key in ["input_ids", "attention_mask", "pixel_values_videos", "video_grid_thw"]:
                multi_modal_inputs[key] = torch.cat([inputs[key] for inputs in micro_batch["multi_modal_inputs"]], dim=0)
            for key in ["second_per_grid_ts"]:
                multi_modal_inputs[key] = [inputs[key][0] for inputs in micro_batch["multi_modal_inputs"]]

        prompts = multi_modal_inputs
        # TODO add response videos, now no videos
        responses = self.processor(text=responses, return_tensors="pt", padding="longest", padding_side="right")
        if n > 1:
            prompts["input_ids"] = prompts["input_ids"].repeat_interleave(n, dim=0)  # [B, L]
            prompts["attention_mask"] = prompts["attention_mask"].repeat_interleave(n, dim=0)  # [B, L]
            pixel_values_videos_list = prompts["pixel_values_videos"].split(prompts["video_grid_thw"].prod(dim=1).tolist())
            prompts["video_grid_thw"] = prompts["video_grid_thw"].repeat_interleave(n, dim=0)  # [BM, 3]
            pixel_values_videos_list = [x for x in pixel_values_videos_list for _ in range(n)]
            prompts["pixel_values_videos"] = torch.cat(pixel_values_videos_list, dim=0)  # [BM*thw, 3*2*14*14]
            prompts["second_per_grid_ts"] = [x for x in prompts["second_per_grid_ts"] for _ in range(n)] # [BM]
        ...
        # position_ids = (attention_mask.cumsum(dim=1) - 1) * attention_mask  # this is for llm, masked
        from verl.models.transformers.qwen2_vl import get_rope_index_batch
        position_ids, rope_deltas = get_rope_index_batch(
            self.processor,
            input_ids=input_ids,
            image_grid_thw=None,
            video_grid_thw=prompts["video_grid_thw"],
            second_per_grid_ts=prompts["second_per_grid_ts"],
            attention_mask=attention_mask,
        )  # (3, bs, seq_len)
        loss_mask = torch.cat([torch.zeros_like(prompts["attention_mask"]), response_mask], dim=1)
        ...
        batch = TensorDict(
            {
                ...
                "position_ids": position_ids.permute(1, 0, 2),  # [bsz, 3, prompt_length + response_length]
            },
            batch_size=len(input_ids),
        )

        print(f'>>> In postprocess, batch=')
        for k, v in batch.items():
            print(f'>>>>>> {k} shape = {v.shape}')
        print(f'>>> In postprocess, multi_modal_inputs= {type(multi_modal_inputs)}')
        ...
        for i in range(len(input_ids)):
            dict_i = {key: multi_modal_inputs[key][i] for key in multi_modal_inputs.keys()}
            multi_modal_inputs_list.append(dict_i)
        multi_modal_inputs = np.array(multi_modal_inputs_list, dtype=object)
        non_tensor_batch = {
            "__num_turns__": num_turns,
            "multi_modal_inputs": multi_modal_inputs, # NOTE that input_ids and attention_mask only for prompt inputs
        }
...
class ChatCompletionScheduler:
    ...
    def submit_chat_completions(self, *, 
        messages: List[Dict[str, str]], 
        extra_body: Dict[str, Any],
        request_id: str, 
        info: Dict[str, Any]):
        ...
        """
        info["__depth__"] += 1
        task = asyncio.create_task(self._submit_chat_completions_and_callback(messages, extra_body, request_id, info))
    ...
    async def _submit_chat_completions_and_callback(
        ...
        if exception is not None:
            ...
            try:
                await self.completion_callback(messages, extra_body, completions, info)
            except Exception as e:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        messages_repeated = batch.non_tensor_batch["messages"].repeat(n, axis=0)
        video_kwargs_repeated = batch.non_tensor_batch["video_kwargs"].repeat(n, axis=0)
        for batch_index, (conversation, video_kwargs) in enumerate(zip(messages_repeated, video_kwargs_repeated)):
            # messages: [{"role": "user", "content": ""}, ["role": "assistant", "content"], ...]
            messages = conversation.tolist()
            extra_body = {
                "mm_processor_kwargs": video_kwargs
            }
            # print(f'>>> In ChatCompletionScheduler prepare messages={messages}')
            # print(f'>>> In ChatCompletionScheduler prepare extra_body={extra_body}')
    ...
    async def _submit_chat_completions_semaphore(self, 
        messages: List[Dict[str, str]], 
        extra_body: Dict[str, Any],
        request_id: str, 
        ...
        self.submit_chat_completions(messages=messages, extra_body=extra_body, request_id=request_id, info=info)
