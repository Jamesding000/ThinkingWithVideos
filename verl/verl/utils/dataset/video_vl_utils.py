...
def apply_draw_number_images(images: list, fps: float, start_time: float = 0.0) -> list:
    """Apply draw number to images.
    Args:
        images (list[Image.Image]): The input images.
        fps (float): the fps of video.
        start_time (float): the start time (in seconds) of the clip.
    Returns:
        list[Image.Image]: The output images.
    """
    num_images = len(images)
    timestamps = [start_time + i / fps for i in range(num_images)]
    timestamps_str = [f"{t:.2f}s" for t in timestamps]
    fontsize = 40
    sqr_size = math.sqrt(images[0].height * images[0].width)
    fontsize = int(fontsize * sqr_size / 336)
    fontpath = 'verl/verl/utils/dataset/DejaVuSans-Bold.ttf'
    font = ImageFont.truetype(fontpath, fontsize)
    margin = 10  
    for image, time_str in zip(images, timestamps_str):
        ...
        draw.text((x, y), time_str, fill=(255, 0, 0), font=font)
    return images
...
def fetch_video(ele: dict, image_factor: int = IMAGE_FACTOR, return_video_sample_fps: bool = False) -> torch.Tensor | list[Image.Image]:
    if isinstance(ele["video"], str):
        ...
        video_list = process_info.pop("video", None)
        fps = process_info.pop("fps", None)
        total_frames = len(video_list)
        # 1. 处理 video_start / video_end，复用 calculate_video_frame_range
        video_start = process_info.get("video_start", None)
        video_end = process_info.get("video_end", None)
        if video_start is not None or video_end is not None:
            if fps is None:
                raise ValueError("fps must be provided when using video_start/video_end with frame list")
            start_frame, end_frame, _ = calculate_video_frame_range(process_info, total_frames, fps)
        ...
        if "draw_number" in process_info and process_info["draw_number"]:
            start_time = process_info.get("video_start", 0.0)
...
def cached_process_vision_info(messages, return_video_kwargs=False):
    ...
    result = process_vision_info(messages, return_video_kwargs)
