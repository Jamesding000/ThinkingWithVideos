        ...
        if self.processor is not None:
            ...
            if self.video_key in row_dict:
                ...
                for video in row_dict.pop(self.video_key):
                    ...
                    # DEBUG: Check video dict before processing
                    print(f"\n=== DEBUG: Before process_video ===")
                    print(f"video_path type: {type(video_path)}")
                    if isinstance(video_path, list):
                        print(f"video_path is list with {len(video_path)} items")
                        print(f"First few items: {video_path[:3]}")
                    else:
                        print(f"video_path: {video_path}")
                    print(f"video dict keys: {video.keys()}")
                    print(f"video['video']: {type(video['video'])}, len={len(video['video']) if isinstance(video['video'], list) else 'N/A'}")
                    print(f"video['fps']: {video.get('fps', 'NOT SET')}")
                    print(f"video.get('nframes'): {video.get('nframes', 'NOT SET')}")
                    print(f"video.get('max_frames'): {video.get('max_frames', 'NOT SET')}")
                    # print(f"self.max_frames_num: {self.max_frames_num}")
                    print(f"=====================================\n")
            ...
            # DEBUG: Print sequence length breakdown
            print(f"\n=== DEBUG: Sequence Length Breakdown ===")
            ...
            if videos is not None:
                ...
                for i, video in enumerate(videos):
                    ...
                    #     # Could be (C, T, H, W) or (T, C, H, W) - check which dimension is 3
                    #     if video.shape[1] == 3:  # (T, C, H, W)
                    #         print(f"  Video {i} frames: {video.shape[0]} (format: T,C,H,W)")
                    #     else:  # (C, T, H, W)
                    #         print(f"  Video {i} frames: {video.shape[1]} (format: C,T,H,W)")
                    # elif len(video.shape) == 5:  # (B, C, T, H, W)
                    #     print(f"  Video {i} frames: {video.shape[2]} (format: B,C,T,H,W)")
            if "pixel_values" in model_inputs:
                print(f"pixel_values shape: {model_inputs['pixel_values'].shape}")
            if "pixel_values_videos" in model_inputs:
                print(f"pixel_values_videos shape: {model_inputs['pixel_values_videos'].shape}")
            if "image_grid_thw" in model_inputs:
                print(f"image_grid_thw: {model_inputs['image_grid_thw']}")
            if "video_grid_thw" in model_inputs:
                print(f"video_grid_thw: {model_inputs['video_grid_thw']}")
            print(f"=====================================\n")
            ...
            # DEBUG: Print sequence length breakdown
            print(f"\n=== DEBUG: Sequence Length Breakdown (no processor) ===")
            print(f"Total input_ids length: {input_ids.shape[1]}")
            print(f"Raw prompt: {raw_prompt[:200]}...")
            print(f"======================================================\n")

        print(f"DEBUG: Before postprocess_data - input_ids length: {input_ids.shape[1]}, max_prompt_length: {self.max_prompt_length}")
