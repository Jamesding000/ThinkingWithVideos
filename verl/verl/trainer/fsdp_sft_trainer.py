...
from transformers import AutoConfig, AutoModelForCausalLM, PreTrainedModel, AutoModelForVision2Seq
...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            ...
            self.model: PreTrainedModel = actor_module_class.from_pretrained(
                local_model_path,
                config=config,
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in tqdm(self.train_dataloader, total=self.steps_per_epoch, desc=f"Epoch {epoch + 1}/{self.config.trainer.total_epochs}", disable=rank != 0):
                ...
                if self.config.trainer.test_freq > 0 and (is_last_step or self.config.trainer.test_freq > 0 and global_step % self.config.trainer.test_freq == 0):
                    # Perform validation
                    val_losses = []
                ...
                if is_last_step or (self.config.trainer.save_freq > 0 and global_step % self.config.trainer.save_freq == 0):
...
def create_sft_dataset(data_paths, data_config, tokenizer, processor):
    ...
    dataset = dataset_cls(data_files=data_paths, tokenizer=tokenizer, config=data_config, processor=processor)
    return dataset
