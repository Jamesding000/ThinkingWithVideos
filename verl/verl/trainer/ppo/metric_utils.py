...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    active_mask = batch.batch["active_mask"]
    turns_stats = batch.batch["turns_stats"]
    valid_action_stats = batch.batch["valid_action_stats"]
    valid_tool_call_stats = batch.batch["valid_tool_call_stats"]
    valid_tool_exec_stats = batch.batch["valid_tool_exec_stats"]
    ...
    metrics = {
        ...
        **(
        ...
        # multi turn
        "multi_turn/active_mask/mean": torch.mean(active_mask.float()).detach().item(),
        "multi_turn/turns_stats/mean": torch.mean(turns_stats.float()).detach().item(),
        "multi_turn/valid_action_stats/mean": torch.mean(valid_action_stats.float()).detach().item(),
        "multi_turn/valid_tool_call_stats/mean": torch.mean(valid_tool_call_stats.float()).detach().item(),
        "multi_turn/valid_tool_exec_stats/mean": torch.mean(valid_tool_exec_stats.float()).detach().item(),
    }
    for k, v in batch.non_tensor_batch.items():
        if "reward_func" in k:
            reward_func = torch.tensor(v)
            metrics[f"critic/rewards/{k}/mean"] = torch.mean(reward_func.float()).detach().item()
        if "custom_reward_func" in k:
            reward_name = batch.non_tensor_batch["reward_name"]
            custom_reward_list = defaultdict(list)
            for name, reward in zip(reward_name, v):
                custom_reward_list[name].append(reward)
            for name, reward_list in custom_reward_list.items():
                reward_tensor = torch.tensor(reward_list)
                metrics[f"critic/rewards/{name}/mean"] = torch.mean(reward_tensor.float()).detach().item()
    return metrics
...
def process_validation_metrics(data_sources: list[str], sample_inputs: list[str], infos_dict: dict[str, list[Any]], seed: int = 42) -> dict[str, dict[str, dict[str, float]]]:
    ...
    sample_inputs = [x + random_str(10) for x in sample_inputs]
    ...
    for data_source, var2metric2prompt_vals in data_src2var2metric2prompt_vals.items():
        for var_name, metric2prompt_vals in var2metric2prompt_vals.items():
            for metric_name, prompt_vals in metric2prompt_vals.items():
                data_src2var2metric2val[data_source][var_name][metric_name] = np.mean(prompt_vals) * 100
                data_src2var2metric2val[data_source][var_name][metric_name + '00001'] = len(prompt_vals)
