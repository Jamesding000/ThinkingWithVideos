...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.rollout.multi_turn.enable:
            ...
            assert config.algorithm.adv_estimator in [AdvantageEstimator.GRPO, AdvantageEstimator.DGRPO], "only GRPO is tested for multi-turn with tool"
    ...
    def _validate(self):
        ...
        if val_data_dir:
            filename = os.path.join(val_data_dir, f"{self.global_steps}_metric.json")
            with open(filename, "w") as f:
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen"):
        ...
        group_size = self.config.actor_rollout_ref.rollout.n  # Batch size = global_batch_size * rollout.n
        image_indices = [idx for idx in all_indices if (idx // group_size) % 4 == 0]
        video_indices = [idx for idx in all_indices if (idx // group_size) % 4 != 0]
        for i, idx in enumerate(image_indices):
            global_partition_lst[i % world_size].append(idx)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                batch_keys_try = ["input_ids", "attention_mask", "position_ids"]
                non_tensor_batch_keys_try = ["multi_modal_data", "mm_processor_kwargs", "raw_prompt", "tools_kwargs", "multi_modal_inputs", "extra_info"]  # for sync mode
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        if not self.async_rollout_mode:
                            with _timer("actor_rollout_wg_gen", timing_raw):
                        ...
                        timing_raw.update(gen_batch_output.meta_info["generation_timing"])
